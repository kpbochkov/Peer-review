 ...
 * Time: 15:12
 */
public abstract class AbstractReadSoftDeleteRepository<T> implements BaseReadRepository<T> {

    private final Class<T> clazz;
    private final SessionFactory sessionFactory;


    public AbstractReadSoftDeleteRepository(Class<T> clazz, SessionFactory sessionFactory) {
        this.clazz = clazz;
    ...
    public <V> T getByField(String name, V value) {
        final String query = String.format("from %s where %s = :value and active = TRUE", clazz.getSimpleName(), name);
    ...
    public List<T> getAll() {
        try (Session session = sessionFactory.openSession()) {
            List<T> list = session.createQuery(String.format("from %s where active = TRUE", clazz.getName()), clazz).list();
            for (T t : list) {
